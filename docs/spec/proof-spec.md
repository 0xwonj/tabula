...
**Decision: Sparse key space with `u64` keys.** Row keys are arbitrary `u64` values (account IDs, item IDs, etc.). The SMT naturally handles sparse keys: unused leaves are default (zero). See §10.2 for rationale.

---

## 4. Cryptographic Building Blocks

### 4.1 Field, Hash, and Poseidon Chip

**Field**: BabyBear (`p = 2^31 - 2^27 + 1 = 2013265921`). See §4.4 for rationale.

**Hash**: Poseidon2 over BabyBear. All in-circuit hashing (state commitments, Merkle trees, IR `Hash` instruction) uses this single configuration:
...
| Partial rounds | 13 |
| Digest | 8 FE (~248 bits, ~124-bit collision resistance) |
| Implementation | `p3_baby_bear::default_babybear_poseidon2_16()` |

A single width-16 permutation is used for both sponge hashing (variable-length input) and Merkle compression (2-to-1).

**Poseidon chip (shared via LogUp)**: Poseidon permutation verification is NOT inlined in each chip. Instead, a dedicated **PoseidonChip** AIR verifies all permutations, and other chips (GlobalSSMC hash chain, GlobalMerge hash chain, IR `Hash` instruction) request permutations via the `PoseidonPermutation` LogUp bus. Each request sends `(input[0..15])` and receives `(output[0..7])`. The PoseidonChip processes one permutation in 21 rows (one per round). This amortizes the constraint cost across all hash invocations and keeps other chips' trace widths smaller.
...
**SSMC Table (AIR trace sub-table)**: Each witness entry becomes a trace row. In practice, all per-(t,c) SSMC rows are merged into a single **GlobalSSMC** table with `is_real` gating — see §4.2.G for the physical global table layout. Logical columns per entry:

| Column | Type | Description |
|--------|------|-------------|
| `key` | u64 | Row key |
| `value` | 𝔽^{w(T)} | Cell value — Tier 1 commitment encoding (§10.3), always non-null |
| `next_key` | u64 | Key of next entry in sorted order (denormalized from next row; required for non-membership gap witness lookups) |
| `is_first` | bool | 1 only for the first entry |
| `is_last` | bool | 1 only for the last entry |

The `next_key` column is a **denormalization** of the next row's `key`. It is constrained to equal the next row's `key` via an AIR transition constraint. This column is necessary for non-membership gap witness LogUp lookups, which require both bounding keys in a single row's fingerprint (LogUp fingerprints reference columns of a single row). See §4.2.G for implementation notes.

**Constraints (enforced in-circuit)**:

1. **Sorted uniqueness**: For each row where `is_last = 0`: `next_key > key` (strict integer inequality). Enforced via the u64 strict inequality gadget (§4.2.R): decompose the difference `next_key - key - 1` into 3 BabyBear limbs, range-check each limb. Cost: O(range-check-gadget) per pair — exact cost depends on Plonky3's range-check implementation, estimated ~8-12 constraints per comparison pending benchmark.
2. **Boundary consistency**: `is_first = 1` only on the first row, `is_last = 1` only on the last row. Where `is_last = 0`, `next_key` equals the following row's `key` (transition constraint).
...
* The prover identifies a **gap witness** — a row in GlobalSSMC (for this `(t,c)`) whose `(key, next_key)` brackets `r`. The proof uses a **single LogUp lookup** with the gap witness tuple plus inequality constraints.
* **Interior gap** (`r` between two entries): Lookup row `i` with tuple `(t, c, key_i, next_key_i, is_first_i, is_last_i)`. The `next_key` column (denormalized from the next row's `key`) is constrained to equal the next row's key via an AIR transition constraint. The inequality constraints `r - key_i - 1 ≥ 0` and `next_key_i - r - 1 ≥ 0` prove `r` falls strictly between two adjacent entries with no entry at `r`. A single LogUp fingerprint references both bounding keys from the same row, which is essential — two separate membership lookups would NOT prove adjacency (an entry at `r` could exist between two non-adjacent looked-up rows).
* **Before first entry** (`r < k_0`): lookup the row where `is_first = 1`, prove `r < key` via strict inequality (`key - r - 1 ≥ 0`).
* **After last entry** (`r > k_{m-1}`): lookup the row where `is_last = 1`, prove `key < r` via strict inequality (`r - key - 1 ≥ 0`).
...
6. **Completeness via LogUp**: Every OldList entry appears exactly once in the merge trace (as `old_only`, `both`, or `delete`). Every WriteSet entry appears exactly once (as `write_only`, `both`, or `delete`). Proved by two LogUp arguments linking OldList and WriteSet to the merge trace. If the OldList is empty (empty column becoming non-empty), the OldList LogUp sum is trivially zero, and the merge trace contains only `write_only` rows. If the WriteSet is empty, the WriteSet LogUp sum is trivially zero, and the merge trace contains only `old_only` rows (no column change).
7. **NewList commitment**: `Poseidon(0x00 || t || c || entries where in_new=1) = Com_new[t,c]`. Only rows with `in_new = 1` contribute to the new hash chain. In the AIR, this is realized via **conditional accumulation**: the hash accumulator column `hash_acc` advances only when `in_new = 1` (i.e., `hash_acc_{i+1} = Poseidon(hash_acc_i || key_{i+1} || new_val_{i+1})`) and carries forward unchanged when `in_new = 0` (i.e., `hash_acc_{i+1} = hash_acc_i`).

**Delete-all case**: When all entries are deleted (every merge row has `is_delete ⟹ in_new = 0`), no rows contribute to the hash chain. The commitment degenerates to `Com_new[t,c] = Poseidon(0x00 || t || c) = Com_empty(t,c)`. The ColumnMeta row for this `(t,c)` must have `is_empty_new = 1` and `Com_new = Com_empty(t,c)`.
...
* **SSMC columns** (`tag=0`, `is_empty_old=0`): `Com_old` equals the hash chain output from the GlobalSSMC segment for `(t,c)`. If `is_touched=1`, `Com_new` equals the hash chain output from the GlobalMerge segment for `(t,c)`. **Wiring mechanism**: Each global table (GlobalSSMC, GlobalMerge) sends a `ColumnMetaJoin` LogUp tuple `(t, c, hash_acc_final)` from the segment-final row (where `is_last=1` for SSMC, last real row in segment for Merge). ColumnMeta receives these tuples and constrains `Com_old = hash_acc_final` (from SSMC) and `Com_new = hash_acc_final` (from Merge). This binding is essential — without it, the prover could place arbitrary values in `Com_old`/`Com_new` that don't match the actual hash chain outputs.
* **SMT columns** (`tag=1`): `Com_old` / `Com_new` equal the column-level SMT roots. Binding is via Merkle path verification against `oldRoot`/`newRoot`.
...
|--------|-------------|
| `t, c` | table and column identifiers |
| `key` | row key `r` (u64 as 3 BabyBear limbs, §4.2.R) |
| `value` | `𝔽^{w(T)}` — Tier 1 commitment encoding (§10.3), always non-null |
| `is_first`, `is_last` | boundary flags |
| `next_key` | u64 (3 limbs) — key of next entry in sorted order (denormalized from next row; see below) |
| `hash_acc` | `𝔽^8` — running Poseidon hash chain accumulator |
| `is_real` | 1 for real rows, 0 for padding |

The `next_key` column is a **denormalization** of the next row's `key`. It is constrained to equal the next row's `key` via an AIR transition constraint (`next_key_i = key_{i+1}` when `is_last=0`). This column exists so that non-membership gap witness LogUp lookups can reference both bounding keys from a single row's fingerprint — LogUp fingerprints reference columns of a single row, so adjacency cannot be proven via two separate membership lookups (see §4.2 non-membership).

> **Implementation note (M8→M10)**: The current codebase (`GlobalSSMCChip`) does not yet include the `next_key` column. It is deferred to M10 when LogUp wiring is implemented. The SSMC chip currently enforces sorted uniqueness via direct `local.key < next.key` transition constraints, which is sufficient for constraint correctness. The `next_key` column will be added when gap witness LogUp lookups are wired.

All SSMC constraints are **gated by `is_real`**:
* Sorted uniqueness: enforced only when `is_real=1` and `is_last=0`
* `next_key` consistency: enforced only when `is_real=1` and `is_last=0`
...
Pairwise constraints (sorted uniqueness, sorted memory transitions) are enforced only when `same_group_{i,i+1} ∧ is_real_i ∧ is_real_{i+1}`.
...
> **Critical soundness dependency.** All ordering and inequality constraints throughout this spec (SSMC sorted uniqueness, SortedMem `(r,τ)` ordering, merge key ordering, gap witnesses) depend on **range checks** on u64 limb decompositions. Without range checks, a malicious prover can satisfy the field-arithmetic equations with out-of-range limb values, making all ordering constraints vacuously satisfiable. Range checks MUST be wired via the RangeCheck LogUp bus before any security claim is made.
...
**Notation convention.** Throughout this spec, any `u64`-typed field in an AIR table (e.g., `r`, `key`, `τ`) is represented as **3 BabyBear limbs** `(x0, x1, x2)` as defined below. Table schemas use the shorthand `key` to mean the 3-limb representation; implementations expand this to three AIR columns (e.g., `key_0, key_1, key_2`).
...
**Strict inequality (x > y).** Decompose the gap `x - y - 1` directly into three non-negative limbs `(d0, d1, d2)` using the borrow-chain gadget: `d0 + d1 · 2^30 + d2 · 2^60 = x - y - 1`. The limbs must be range-checked (`d0, d1 ∈ [0, 2^30)`, `d2 ∈ [0, 16)`). If the range checks pass, `x - y - 1 ≥ 0`, i.e., `x > y`.
...
* "sorted uniqueness" (`next_key > key`): strict inequality gadget on u64 limbs; `next_key` is constrained to equal the next row's `key` via transition constraint
* "gap witness" (`key < r < next_key`): two strict inequality checks on u64 limbs, referencing `key` and `next_key` from the same row via a single LogUp gap witness lookup
...
Inter-tx memory read/write consistency is verified using **LogUp**. See §4.5 for the LogUp argument definition and fingerprint construction, and §8 for the full memory consistency design. Intra-tx consistency requires no RAM argument (§6.3.3).
...
### 4.6 Chip and Bus Architecture Overview

The proof consists of multiple AIR chips connected via LogUp buses. Each chip is a separate fixed-width trace; inter-chip communication uses LogUp multiset arguments.

**Chips**:

| Chip | Role | Section |
|------|------|---------|
| ExecutionChip | Instruction trace (one row per instruction), SSA slots, access log | §6, §8.7 |
| GlobalSSMC | Per-(t,c) sorted SSMC witness + hash chain | §4.2 |
| GlobalMerge | Per-(t,c) merge trace (OldList + WriteSet → NewList) | §4.2 |
| GlobalSortedMem | Sorted memory events for inter-tx consistency | §8 |
| ColumnMeta | Binds Com[t,c] to root inclusion proofs + metadata | §4.2 ColumnMeta |
| PoseidonChip | Shared Poseidon2 permutation verification | §4.1 |
| RangeCheckChip | Preprocessed [0, 2^16) lookup table for integer gadgets | §4.2.R |

**LogUp buses**:

| Bus | Send side | Receive side |
|-----|-----------|--------------|
| Memory | ExecutionChip (access log) | GlobalSortedMem (non-init rows) |
| SsmcMembership | GlobalSortedMem (init rows) | GlobalSSMC (membership/gap witness) |
| MergeCompleteness | GlobalSortedMem (UpdateSet) + GlobalSSMC (OldList) | GlobalMerge (write/old entries) |
| ColumnMetaJoin | GlobalSSMC / GlobalMerge (segment-final hash) | ColumnMeta (Com_old / Com_new) |
| PoseidonPermutation | GlobalSSMC / GlobalMerge / ExecutionChip | PoseidonChip |
| RangeCheck | All chips with u64 limbs | RangeCheckChip |

Each bus uses independent Fiat-Shamir challenges for domain separation. Detailed fingerprint definitions are given in the sections referenced above. For the full chip implementation architecture, see `docs/air-chip-architecture.md`.
...
| `ProgramRoot` | **Required** | Commitment (Merkle root) to the set of allowed tx type definitions. Each tx in the batch must prove its tx type is included via a Merkle inclusion proof against `ProgramRoot`. |
| `StaticTableRoot` | **Required** | Commitment (Merkle root) to all static (immutable) tables used by `Lookup` instructions. `Lookup` results are verified via LogUp/Lasso arguments against this root. |
...
* **SSMC columns**: The entire column is laid out as an SSMC Table (§4.2). Membership: LogUp lookup proves `(r, init_value)` exists in the table. Non-membership: a single LogUp gap witness lookup referencing `(key, next_key)` from the bounding row, plus strict inequality constraints `key < r < next_key` (or boundary case via `is_first`/`is_last`), per §4.2. The SSMC table's sorted uniqueness and commitment hash are verified once; individual openings use LogUp.
...
**UpdateSet → Merge WriteSet link (LogUp).** For SSMC columns, the UpdateSet must match the merge trace's WriteSet entries exactly. This is enforced via a `MergeCompleteness` LogUp argument:

* **Send side** (GlobalSortedMem): each row satisfying `is_real ∧ is_last_for_key ∧ has_written` sends the tuple `(t, c, r, mem, mem_is_null)`.
* **Receive side** (GlobalMerge): each row with `source ∈ {write_only, both, delete}` receives the tuple `(t, c, key, write_val, write_is_null)` where `write_is_null = 1` for `delete` rows and `0` otherwise.
* LogUp multiset equality ensures every extracted write appears exactly once in the merge trace, and vice versa.

For SMT columns, the UpdateSet entries are applied directly as Merkle path updates (no merge trace).
...
* **SSMC** (columns ≤ threshold): Sorted sparse map commitment. Prover provides entire column as a sorted AIR trace sub-table with `(key, value, next_key, is_first, is_last)` columns. In-circuit: sorted uniqueness via transition constraints + range checks, domain-separated Poseidon hash chain for commitment, per-cell membership/non-membership via **LogUp lookups** into the SSMC table (not array indexing). Update correctness via **3-way merge proof** (OldList + WriteSet → NewList). Cost: O(m) setup + O(LogUp) per access + O(m+w) for updates.
...
**Decision: Global monotone counter, bound to instruction ordering by AIR clock constraints.** See §8.7 for the full definition (`τ = clk_i + 1` for access events, `τ = 0` for init rows).
...
**Decision: No intra-tx RAM-consistency argument of any kind.** Enforced by the canonical state normal form ([semantics-spec.md](./semantics-spec.md) §2) and True SSA (§1.2). See §6.3.3 for the full argument. Inter-tx consistency is handled separately by the per-(t,c) memory argument (§8).

### 10.7 In-Circuit Hash Strategy

**Decision: Poseidon2 over BabyBear for all in-circuit hashing.** See §4.1 for the full configuration (width=16, rate=8, S-box=x^7, 21 rounds). Verified by a dedicated PoseidonChip shared via LogUp (§4.1, §4.6).

* **Canonical `hash_id`**: See [semantics-spec.md](./semantics-spec.md) §1.7 for the semantic definition. Blake3 = non-canonical debug mode only.
...
| Hash function | **Poseidon2** over BabyBear (width=16, rate=8, S-box=x^7, 21 rounds); Blake3 = non-canonical debug mode only (§4.1) |
| Poseidon chip | Dedicated PoseidonChip AIR shared via `PoseidonPermutation` LogUp bus (§4.1, §4.6) |
| Memory consistency | **LogUp** argument (~5-15 constraints/access), per-(t,c) SortedMemTable |
| Multi-chip architecture | 7 AIR chips connected via 6 LogUp buses (§4.6) |
