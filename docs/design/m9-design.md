...
### 8.2 PoseidonChip (69 → 93)

| Change | Columns | Reason |
|--------|---------|--------|
| `perm_input[16]` | +16 | Poseidon bus input fingerprint |
| `perm_output[8]` | +8 | Permutation digest carry |
| **Subtotal** | **+24** | **69 → 93** |

Note: 17-column preprocessed trace (`rc[16]` + `is_full_round`) added in A2, not counted in main trace width.
...
+121 columns across all chips (Execution +52, Poseidon +24, SSMC +18, Merge +17,
SortedMem +9, ColumnMeta +1). Plus per-chip permutation trace columns (extension field,
generated by prover, not part of main trace width). PoseidonChip also has a 17-column
preprocessed trace for round constant verification.
...
- [x] ~70 new M9 tests pass (LogUp + local constraints) — 250 total (46 new)
- [x] `check_logup_balance()` verifies LogUp balance for a full batch
- [x] All chips emit send/receive interactions via InteractionAirBuilder
- [x] Operand-to-slot linkage constrains `src1_val`, `src2_val`, `cond_val`
- [x] Poseidon RC verified against preprocessed constants (17-col preprocessed trace)
- [x] SSMC/Merge hash chains constrained via PoseidonPermutation bus
- [x] SSMC/Merge commitments verified against ColumnMeta via CommitmentVerification bus
- [x] SortedMem `meta_is_empty_old` verified via SortedMemMeta bus
- [x] Memory bus links Execution to SortedMem
- [x] MergeOldList + MergeWriteSet buses enforce merge completeness
- [x] `cargo test -p tabula-proof --features stark` passes (250 tests)
- [x] `cargo clippy -p tabula-proof --features stark --all-targets` — zero warnings
- [x] Design doc column widths corrected, MEMORY.md updated
