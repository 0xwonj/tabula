...
├── witness/
│   ├── mod.rs
│   ├── types.rs          ← BatchWitness, InitRow, AccessRow, ColumnWitness
│   ├── generator.rs      ← WitnessGenerator
│   ├── route.rs           ← KeyRoute + route_keys() (Phase 1 rename done)
│   └── program_info.rs    ← ProgramInfo, LiteralCell, TemplateId (types only)
└── air/
    ├── mod.rs
    ├── bus.rs            ← InteractionKind (8 variants, not yet wired)
    ├── columns.rs
    ├── debug.rs
    ├── gadgets/
    │   ├── mod.rs
    ...
    │   └── mem.rs        ← null_canon, mem_read, mem_write
    └── chips/
...
Naming conventions established. `KeyRoute`, `route_keys()`, `ProgramInfo` types in place.

| Action | Detail | Status |
|--------|--------|--------|
| `classify.rs` → `route.rs` | Rename file, `AccessClass` → `KeyRoute`, `classify_keys` → `route_keys` | ✅ |
| `types.rs` | `key_classification` → `key_routes` | ✅ |
| `witness/`, `lib.rs` | Module restructured, imports and re-exports updated | ✅ |
