...
pub fn compute_fingerprint<F: Field>(values: &[F], bus: BusId, alpha: F, beta: F) -> F {
    let mut result = alpha + F::from_u64(bus.tag() as u64);
...
fn accumulate_logup_sum<F: Field>(
    ...
    for record in records {
        for interaction in &record.interactions {
            ...
            let fingerprint =
                compute_fingerprint(&interaction.values, interaction.bus, alpha, beta);

            if fingerprint == F::ZERO {
                return Err(MultiChipError::LogUpImbalance {
                    description: format!(
                        "zero fingerprint in chip '{}' for bus {}",
                        record.name, interaction.bus
                    ),
                });
            }
