...
impl<'a> ProofInstance<'a> {
    ...
    pub fn commit_main(&mut self) -> Result<MainCommitment, ProveError> {
        ...
        let (preprocessed_commitment, preprocessed_data): (
            Option<PcsCommitment>,
            Option<PcsProverData>,
        ) = if !pp_chip_indices.is_empty() {
    ...
    pub fn prove(mut self, challenger: &mut Challenger) -> Result<SubProof, ProveError> {
        ...
        let (perm_commitment, perm_data): (Option<PcsCommitment>, Option<PcsProverData>) =
            if !rap_chip_indices.is_empty() {
