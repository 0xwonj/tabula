...
impl Sdk {
    ...
    fn build_executor(
        ...
        Ok(tabula_runtime::prepare_executor(registered, &opts)?)
    }
    ...
    fn build_prepared_verifier(
        ...
        let opts = self.prepared_options()?;
        Ok(tabula_runtime::prepare_verifier(sealed, &opts)?)
    }
    ...
    fn build_prover(
        ...
        let registered = Arc::new(artifact.registered().clone());
        let opts = self.prepared_options()?;
        Ok(tabula_runtime::prepare_prover(registered, &opts)?)
    }
...
"#;
    ...
    fn prepare_executor_build_failure_does_not_poison_cache() {
        ...
        assert!(matches!(
            first,
            SdkError::Runtime(RuntimeError::Setup(_) | RuntimeError::Execute(_))
        ));
        assert!(matches!(
            second,
            SdkError::Runtime(RuntimeError::Setup(_) | RuntimeError::Execute(_))
    ...
    fn prepare_prover_build_failure_does_not_poison_cache() {
        ...
        assert!(matches!(
            first,
            SdkError::Runtime(RuntimeError::Setup(_) | RuntimeError::Prove(_))
        ));
        assert!(matches!(
            second,
            SdkError::Runtime(RuntimeError::Setup(_) | RuntimeError::Prove(_))
    ...
    fn prepare_prepared_verifier_build_failure_does_not_poison_cache() {
        ...
        assert!(matches!(
            first,
            SdkError::Runtime(RuntimeError::Setup(_) | RuntimeError::Verify(_))
        ));
        assert!(matches!(
            second,
            SdkError::Runtime(RuntimeError::Setup(_) | RuntimeError::Verify(_))
