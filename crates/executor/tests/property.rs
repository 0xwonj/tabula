...
use common::{XorHasher, committed_u64_state, property_program, test_state_runtime, type_runtimes};
...
fn property_read_minimum_records_effect_and_returns_row_tuple() {
    ...
    assert_eq!(
        result.property_effects[0].result,
        tabula_types::TypedCommittedPropertyQueryResult {
            value: u64_typed(50),
            key: Some(tabula_core::CommittedKey(5u64.to_le_bytes().to_vec())),
            is_null: false,
        }
    );
}
...
fn property_read_aggregate_is_unsupported_in_v1_adapter() {
    ...
    assert!(
        error
            .error
            .to_string()
            .contains("Aggregate is not yet supported in test state runtime")
    );
}
...
fn property_read_non_existence_range_is_unsupported_in_v1_adapter() {
    ...
    assert!(
        error
            .error
            .to_string()
            .contains("NonExistenceRange is not yet supported in test state runtime")
    );
}

#[test]
fn property_read_uses_state_runtime_key_schema_instead_of_raw_ir_key_type() {
    let runtimes = type_runtimes();
    let exec = exec::ExecContext {
        hasher: &XorHasher,
        type_runtimes: &runtimes,
    ...
    let result = exec::execute_query(&program, ir::EntryId(0), &[], &context, &state, &exec)
        .expect("state runtime key schema should remain authoritative");
    assert_eq!(
        result.returns,
        vec![u64_typed(0), u64_typed(0), bool_typed(true)]
    );
