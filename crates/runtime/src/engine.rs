...
// TODO(SP-5): remove these re-exports once the `runtime_root_exposes_only_the_final_native_surface`
// architecture test relaxes the literal-string pin on the `pub use engine::{...}` line in
// `crates/runtime/src/lib.rs`. Until then, the relocated types must be reachable through
// `engine::` to keep that pin satisfied.
pub use crate::snapshot::CommittedStateSnapshot;
// TODO(SP-5): remove once architecture pin relaxes
pub use crate::execution::ExecutionReceipt;
use crate::prepared_state::{PreparedRuntimeState, build_prepared_runtime};
#[cfg(feature = "prove")]
use crate::prepared_state::ColumnProofSlot;
...
impl TabulaRuntime {
    ...
    {
        crate::execution::materialize_logical_state(&self.runtime_program, cells)
    }
    ...
    ) -> Result<Vec<crate::snapshot::LogicalStateCell>, RuntimeError> {
        crate::execution::project_logical_state(&self.runtime_program, snapshot)
    }
    ...
    pub fn execute_batch(
        ...
        crate::execution::execute_batch(&self.runtime_program, snapshot, batch, context)
    }
    ...
    pub fn execute_batch_receipt(
        ...
        crate::execution::execute_batch_receipt(&self.runtime_program, snapshot, batch, context)
    }
...
"#
    ...
    fn lowering_rejects_duplicate_relation_effect_origins() {
        ...
        let typed_context =
            crate::prelude::decode_context_input_on_state(&runtime.runtime_program, &context)
                .expect("typed context");
        let typed_txs =
            crate::prelude::decode_entry_batch_on_state(&runtime.runtime_program, &batch)
                .expect("typed batch");
        let entry = runtime
            .execution_program()
