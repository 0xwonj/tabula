...
use tabula_testing::witness::{
    build_trace_map, compile_execute_case, lower_program_batch_for_harness,
    prepare_execution_inputs, prepare_witness_store,
};
...
fn stark_trace_builder_executes_all_core_chips_e2e() {
    ...
    let column = prepared
        .column(TableId(0), ColId(0))
        .expect("prepared column");

    assert_eq!(column.access_events.len(), 2);
    assert_eq!(column.access_events[0].effect_ordinal_in_tx, 0);
    assert_eq!(column.access_events[1].effect_ordinal_in_tx, 1);
