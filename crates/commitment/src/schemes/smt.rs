...
/// A sparse Merkle tree with configurable depth and proof-compatible domain
/// separation.
///
/// Stores only non-empty nodes. Empty subtrees use precomputed empty hashes,
/// seeded by the tree's domain tag. Internal nodes intentionally use plain
/// 2-to-1 compression so the native tree matches the current proof chips.
...
impl<H: FieldHasher> SparseMerkleTree<H> {
    ...
    pub fn insert(&mut self, key: u64, value: H::Digest) -> Result<H::Digest, TabulaError> {
    ...
    pub fn remove(&mut self, key: u64) -> Result<H::Digest, TabulaError> {
        self.validate_key(key)?;
    ...
    pub fn prove(&self, key: u64) -> Result<MerkleProof<H::Digest>, TabulaError> {
        self.validate_key(key)?;
    ...
    fn validate_key(&self, key: u64) -> Result<(), TabulaError> {
        let max_key_exclusive = 1u64
            .checked_shl(self.depth as u32)
            .expect("SMT depths must fit within u64");
        if key < max_key_exclusive {
            return Ok(());
        }
        Err(TabulaError::ProofError {
            phase: "commitment",
            detail: format!(
                "SMT key {key} out of range for depth {} (max allowed: {})",
                self.depth,
                max_key_exclusive - 1
            ),
        })
...
mod tests {
    use super::*;
    use crate::primitives::{DOMAIN_COL, DOMAIN_SMT, DOMAIN_TABLE, MockFieldHasher};
    ...
    fn prove_absent_key_non_membership() {
        ...
        tree.insert(0, leaf).unwrap();
        let proof = tree.prove(1).unwrap(); // key 1 not inserted
    ...
    fn different_domain_tags_different_roots() {
        ...
        let leaf = MockFieldHasher.hash(&[p3_koala_bear::KoalaBear::new(1)]);
        t1.insert(0, leaf).unwrap();
        t2.insert(0, leaf).unwrap();
    ...
    fn out_of_range_keys_are_rejected() {
        let mut tree = make_tree(8, DOMAIN_SMT);
        let leaf = MockFieldHasher.hash(&[p3_koala_bear::KoalaBear::new(7)]);
        let out_of_range = 1u64 << 8;

        assert!(tree.insert(out_of_range, leaf).is_err());
        assert!(tree.remove(out_of_range).is_err());
        assert!(tree.prove(out_of_range).is_err());
    }
}
