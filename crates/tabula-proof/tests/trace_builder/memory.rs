...
fn trace_builder_builds_valid_memory_traces() {
    ...
    let ito_trace = &trace_map.get("InterTxOrder").expect("InterTxOrder").main;
    let state_trace = &trace_map.get("StateColumn").expect("StateColumn").main;
    let cm_trace = &trace_map.get("ColumnMeta").expect("ColumnMeta").main;
