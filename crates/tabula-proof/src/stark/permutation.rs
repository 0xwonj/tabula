...
pub(crate) fn compute_fingerprint_ef4(
    ...
    let mut result = alpha + EF4::from(BabyBear::from_u64(kind.tag() as u64));
...
fn compute_chip_cumsum(
    ...
    for interaction in interactions {
        ...
        let fingerprint =
            compute_fingerprint_ef4(&interaction.values, interaction.kind, alpha, beta);
...
mod tests {
    ...
    fn fingerprint_ef4_deterministic() {
        ...
        let f1 = compute_fingerprint_ef4(&values, InteractionKind::ReadAccess, alpha, beta);
        let f2 = compute_fingerprint_ef4(&values, InteractionKind::ReadAccess, alpha, beta);
        assert_eq!(f1, f2);
    }
    ...
    fn fingerprint_ef4_different_bus() {
        ...
        let f1 = compute_fingerprint_ef4(&values, InteractionKind::ReadAccess, alpha, beta);
        let f2 = compute_fingerprint_ef4(&values, InteractionKind::RangeCheck, alpha, beta);
        assert_ne!(f1, f2);
    }
