...
use crate::air::chips::range_check::RANGE_CHECK_SIZE;
use crate::air::debug::ChipRecord;
...
pub(super) fn collect_poseidon_inputs(
    ...
    for record in records {
        for interaction in &record.interactions {
            if interaction.kind != InteractionKind::PoseidonPermutation
...
pub(super) fn collect_range_check_multiplicities(
    ...
    for record in records {
        for interaction in &record.interactions {
            if interaction.kind != InteractionKind::RangeCheck
                || interaction.direction != InteractionDirection::Send
            {
                continue;
            }
